...
# driving time summary - NA duration analysis
## Are there NA?
yes.  From 5 distinct meshblocks / ID
...
```
## Are they really NA driving duration?
calculate them again - yes
...
How far between these NA duration ids and meshblocks
Totalling 457M, or 100M average

Also note, these are the closest meshblocks to each of these testing locations
...
df_driving_time %>%
  filter(is.na( duration)) %>%
  inner_join(  df_crow_distances, by = c("id", "MB_CODE16") ) %>%
  ungroup() %>%
  summarise( sum(dist))
...
## Duration between each testing station and closest meshblock (by duration)
